...
export const init = async model => {
   ...
   let strokes_rotXZ = (src, theta) => {
      let C = Math.cos(theta), S = Math.sin(theta);
      let dst = [];
      for (let n = 0 ; n < src.length ; n++) {
         dst[n] = [];
         for (let i = 0 ; i < s1[n].length ; i++)
            dst[n][i] = [ S * src[n][i][0], src[n][i][1], C * src[n][i][2] ];
      }
      return dst;
   }

   let strokes_xform = (src, xyzs) => {
      let dst = [];
      for (let n = 0 ; n < src.length ; n++) {
         dst[n] = [];
         for (let i = 0 ; i < src[n].length ; i++)
            dst[n][i] = [ xyzs[0] + xyzs[3] * src[n][i][0],
   ...
   matchCurves.addGlyphFromCurves(
      '\\',
      s1,
      (time, xyzs) => strokes_xform(strokes_rotXZ(s1, time + Math.PI/4), xyzs)
   );
