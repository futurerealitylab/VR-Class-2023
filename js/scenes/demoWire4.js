	 ...
	 strokes[strokes.length-1].push(cg.mTransform(m, [0,-.05,-.1]));
      ...
      for (let n = 0 ; n < strokes.length ; n++)
         if (strokes[n].length > 1) {
            let outer = wires.add(clay.wire(strokes[n].length, 8, n));
            clay.animateWire(outer, .010, t => cg.sample(strokes[n], t));
            let inner = wires.add(clay.wire(strokes[n].length, 8, n + 100));
            clay.animateWire(inner, .005, t => cg.sample(strokes[n], t));
         }
