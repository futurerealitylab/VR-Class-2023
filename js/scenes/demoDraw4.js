...
window.drawClients = [];
for (let n = 0 ; n < 10 ; n++)
   drawClients.push(new DrawClient());
...
export const init = async model => {
   ...
   let wires = [], ST = [], timer = [];
   for (let id = 0 ; id < drawClients.length ; id++)
      wires[id] = model.add();
   ...
   let buildWires = (id, strokes) => {
      model.remove(wires[id]);
      wires[id] = model.add();
      let W = wires[id];

      let D = drawClients[id];
      for (let n = 0 ; n < strokes.length ; n++)
         if (strokes[n].length > 1) {
            let outer = W.add(clay.wire(strokes[n].length, 6, n));
            let inner = W.add(clay.wire(strokes[n].length, 6, n + 100));
   ...
   let isLeftDown = false, isRightDown = false, D;
   ...
   inputEvents.onDrag = hand => {
      if (hand == 'right') {
         if (! isLeftDown && D.strokes.length > 0) {
	    let p = inputEvents.pos('right');
	    for (let i = 0 ; i < p.length ; i++)
	       p[i] = (p[i] * 100 >> 0) / 100;
            D.strokes[D.strokes.length-1].push(p);
         }
      }
   }
   ...
   inputEvents.onRelease = hand => {
      if (hand == 'left') {
         isLeftDown = false;
         if (D.strokes.length > 0 && D.strokes[0].length > 1) {
            D.mode = 'morph';
            server.broadcastGlobalSlice('drawClients', clientID, clientID+1);
         }
      }
      if (hand == 'right') {
         isRightDown = false;
         if (D.mode == 'morph' || D.strokes.length > 0 && D.strokes[D.strokes.length-1].length < 10)
	    D.mode = 'delete';
         server.broadcastGlobalSlice('drawClients', clientID, clientID+1);
      }
   }
   ...
   model.animate(() => {
      drawClients = server.synchronize('drawClients');

      if (window.clientID === undefined)
         return; 
      if (D === undefined)
         D = drawClients[clientID];
	 ...
	 switch (D.mode) {
         case 'morph':
	    if (! ST[id]) {
               ST[id] = matchCurves.recognize(D.strokes);
	       timer[id] = 0;
            }
            timer[id] += 1.8 * model.deltaTime;
            if (timer[id] < 1)                       // MORPH AFTER RECOGNIZING
               strokes = matchCurves.mix(ST[id][0], ST[id][1], cg.ease(timer[id]));
            else {                                   // ANIMATE AFTER MORPHING
               let glyph = matchCurves.glyph(ST[id][2]);
               if (glyph.code)
                  strokes = glyph.code(timer[id] - 1, ST[id][3]);
            }
	    break;
	 ...
	 case 'delete':
            D.mode = null;
            D.strokes = [];
            server.broadcastGlobalSlice('drawClients', id, id+1);
            ST[id] = null;
            strokes = [];
	    break;
         }

	 buildWires(id, strokes);
      }
   });
}
