import wasmLoader      from "/wasm/main.js";
import * as wasmGlobal from "/js/wasm.js";
...
function wasmLoad() {
   return wasmLoader().then((code)=> {
      wasmGlobal.wasmSetModule(code);
      const statusCode = code._setup();
      if (statusCode == 0) {
         console.log("wasm setup successful");
      }
   }).catch((err) => {
      console.error(err);
   })
...

{
   ...
   textureCanvas.width = 1024 / 2;
   textureCanvas.height = 1024 / 2;
...
async function main() {
   wasmLoad().then(() => {
      import("/js/corelink_handler.js").then(async (mod) => {
         await mod.run();
         await import("/js/util/voip-webrtc.js");

         console.log("isSecureContext=[" + isSecureContext + "] crossOriginIsolated=[" + crossOriginIsolated + "]");
      });
   });
}
