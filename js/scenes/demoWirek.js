...
export const init = async model => {
   ...
   let wasLeftPinch = false, wasRightPinch = false;
   ...
   model.animate(() => {
      ...
      let eventType = controllerEventType();


      let leftPos = cg.mTransform(cg.mMultiply(controllerMatrix.left,
                                  cg.mInverse(model.getMatrix())), [0,-.05,-.1]);

      let rightPos = cg.mTransform(cg.mMultiply(controllerMatrix.right,
                                   cg.mInverse(model.getMatrix())), [0,-.05,-.1]);


      let isLeftPinch = clay.handsWidget.pinch.left == 1;
      if (isLeftPinch) {
         eventType = 'leftTriggerDrag';
         leftPos = clay.handsWidget.getMatrix('left', 1, 4).slice(12,15);
      }
      if (isLeftPinch && ! wasLeftPinch) eventType = 'leftTriggerPress';
      if (wasLeftPinch && ! isLeftPinch) eventType = 'leftTriggerRelease';
      wasLeftPinch = isLeftPinch;



      let isRightPinch = clay.handsWidget.pinch.right == 1;
      if (isRightPinch) {
         eventType = 'rightTriggerDrag';
         rightPos = clay.handsWidget.getMatrix('right', 1, 4).slice(12,15);
      }
      if (isRightPinch && ! wasRightPinch) eventType = 'rightTriggerPress';
      if (wasRightPinch && ! isRightPinch) eventType = 'rightTriggerRelease';
      wasRightPinch = isRightPinch;


      pinchStr += isLeftPinch ? 'L' : isRightPinch ? 'R' : '-';
      console.log(pinchStr);
      ...
      case 'rightTriggerDrag':
         strokes[strokes.length-1].push(rightPos);
         buildWires();
         break;
