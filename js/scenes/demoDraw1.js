...
export const init = async model => {
   ...
   let wasPinchL = false, wasPinchR = false;

   model.animate(() => {
      helpMenu.identity().move(.65,1.5,.5).turnY(-.8).scale(.25,.25,.0001);


      let pinchL = clay.handsWidget.pinch.left  == 1;
      let pinchR = clay.handsWidget.pinch.right == 1;
      let pointL = clay.handsWidget.getMatrix('left' ,1,4).slice(12,15);
      let pointR = clay.handsWidget.getMatrix('right',1,4).slice(12,15);

      let pressL   = pinchL && ! wasPinchL;
      let pressR   = pinchR && ! wasPinchR;
      let releaseL = wasPinchL && ! pinchL;
      let releaseR = wasPinchR && ! pinchR;
