...
export const init = async model => {
   ...
   let colors = ['red','#ff4000','yellow','#40ff40','#2080ff','#9000ff','#ff00a0'];
   ...
   let helpMenu = model.add('cube').texture(() => {
      ...
      g2.setColor('#ff80ff');
      g2.textHeight(.07);
      g2.fillText('Things you can draw', 0, .9, 'left');

      g2.textHeight(.05);
      g2.fillText('Stroke order:', 0, .82, 'left');
      for (let i = 0 ; i < colors.length ; i++) {
         g2.setColor(colors[i]);
         g2.fillRect(.31 + i * .05, .80, .04, .04);
      }

      g2.setColor('#ff80ff');
      g2.fillText('To draw: Hold down the right trigger', 0, .155, 'left');
      g2.fillText('To erase: Click the right trigger'   , 0, .095, 'left');
      g2.fillText('To animate: Click the left trigger'  , 0, .035, 'left');
      g2.fillText('To draw:'   , .002, .155, 'left');
      g2.fillText('To erase:'  , .002, .095, 'left');
      g2.fillText('To animate:', .002, .035, 'left');
   ...
   model.animate(() => {
	 ...
	 let m = cg.mMultiply(controllerMatrix.right,
      ...
      case 'rightTriggerRelease':
         if (mode == 'morph' || strokes[strokes.length-1].length < 10) {
            ST = null;
            mode = null;
            strokes = [];
            buildWires();
