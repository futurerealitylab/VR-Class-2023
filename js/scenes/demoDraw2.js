...
export const init = async model => {
   ...
   let wasDragL = false, wasDragR = false;

   model.animate(() => {
      helpMenu.identity().move(.65,1.5,.5).turnY(-.8).scale(.25,.25,.0001);


      let dragL, dragR, pointL, pointR, pressL, pressR, releaseL, releaseR;

      if (window.handtracking) { 
         pointL = clay.handsWidget.getMatrix('left' ,1,4).slice(12,15);
         pointR = clay.handsWidget.getMatrix('right',1,4).slice(12,15);

         dragL  = clay.handsWidget.pinch.left  == 1;
         dragR  = clay.handsWidget.pinch.right == 1;

         pressL   = dragL && ! wasDragL;
         pressR   = dragR && ! wasDragR;
         releaseL = wasDragL && ! dragL;
         releaseR = wasDragR && ! dragR;

         wasDragL = dragL;
         wasDragR = dragR;
      }
      else {
         pointL = cg.mTransform(cg.mMultiply(controllerMatrix.left , cg.mInverse(model.getMatrix())), [0,-.05,-.1]);
         pointR = cg.mTransform(cg.mMultiply(controllerMatrix.right, cg.mInverse(model.getMatrix())), [0,-.05,-.1]);

	 switch (controllerEventType()) {
	 case 'leftTriggerPress'   : dragL = pressL = true; break;
	 case 'rightTriggerPress'  : dragR = pressR = true; break;
	 case 'leftTriggerDrag'    : dragL = true; break;
	 case 'rightTriggerDrag'   : dragR = true; break;
	 case 'leftTriggerRelease' : releaseL = true; break;
	 case 'rightTriggerRelease': releaseR = true; break;
	 }
      }
      ...
      if (dragR) {
         if (strokes.length > 0) {
            strokes[strokes.length-1].push(pointR);
            buildWires();
         }
      }
